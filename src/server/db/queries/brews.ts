...
const one = (subquery: number, brewid: number) => Query(`select brews.id, brews._createdat, brews.barista, brews.roasteddate, brews.grindsize, brews.gramspregrind, brews.gramspostgrind, brews.watertempprebrew, brews.watertemppostbrew, brews.bloomtimeinsec, brews.bloomweight, brews.brewtimeinsec, brews.drawdownstart, brews.brewweight, brews.yeild, coffeebags.id as coffeebagid, coffeebags.brand as coffeebrand, coffeebags.name as coffeename, filters.brand_name_style as filter, grinders.name as grinder, brewmethods.name as brewmethod, brands.name as brandname from brews join brands on brands.id = (select coffeebags.brand from coffeebags where coffeebags.id = (select brews.coffeebag where brews.id = ?)) join filters on filters.id = brews.filter join coffeebags on coffeebags.id = brews.coffeebag join brewmethods on brewmethods.id = brews.brewmethod join grinders on grinders.id = brews.grinder where brews.id = ?`, [subquery, brewid]);
